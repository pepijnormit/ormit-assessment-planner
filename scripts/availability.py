...
def get_calender(ics_url, staff_name, start_date_string, end_date_string):
    ...
    if response.status_code == 200:
        ...
        # # Read the calendar file and parse it with icalendar
        ...
        # Drop rows where the Event Name is 'Free'
        df = df[df['Event Name'] != 'Free']
...
def get_free_time(schedule_df, staff_member):
    ...
    for day in schedule_df['Start Date'].dt.date.unique():
        events_for_day = schedule_df[schedule_df['Start Date'].dt.date == day]
    ...
    for key in excel_file.keys():   #For all assessors in assessor file
        if key != 'Extra' and key != 'External':
            if key in calenders.keys(): #If calender link is found
                schedule = get_calender(calenders, key, str(start_date), str(end_date))
                avail_gen = get_free_time(schedule, key)
